    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            if torch.sum(torch.isnan(q)).item() > 0:
                print(torch.mean(query, dim=-1))
                print('error before in query', torch.sum(torch.isnan(q), dim=-1).squeeze())
            if torch.sum(torch.isnan(k)).item() > 0:
                print('error before in key', torch.sum(torch.isnan(k), dim=-1).squeeze())
            if torch.sum(torch.isnan(v)).item() > 0:
                print('error before in value', torch.sum(torch.isnan(v), dim=-1).squeeze())
